...
After exploring the associations between the independent variables and the response, I am thinking
a non-linear modeling approach will be the most accurate. There are many complicated trends in the data just looking at
univariate relationships with the response and without looking at any variable interactions. I'll still try Logistic regression, but I have a feeling KNN/ Random Forest may perform better. <br>
<br>

The code behind the functions used in this section can be found at the bottom of the page.
...
**capital_gain** <br>
In this plot, capital gain is log scaled. The y-axis is in a probability form instead of log-odds.  I added an interaction with an indicator telling the model if the value is 0 or otherwise. It looks like there is great separation between capital gain and over_50k here. The line between x = 0 and x = 3 only looks like it is above y = 0 because there were no capital gain values between that range. If there were then the S shape would stay at y = 0, until x hit 0 
```{r}
train %>% 
...
**capital_loss** <br>
It looks like the important distinction between this variable is that at 0, it is more likely to make less than 50k. Greater than 0 it is more likely to make 50k. Next I'll filter out the zeros so I can better see the middle distribtion.
```{r}
...
```{r}
plot_continuous(train$hours_week, n = 30, response = train$over_50k, 
...
**hours_week** <br>
The best way to fit the data I've found is to add an indicator variable if the value is over 50 or not, and add an interaction effect. I also considered condensing all the values past 50 to 50, but I don't think that would have been a good long term solution.
```{r}
...
train %>% 
  ...
  summarise(pct_over_50k = sum(over_50k)/n(),
            n = n()) %>% 
  arrange(pct_over_50k) %>%
  mutate(workclass = factor(workclass, levels = unique(workclass))) %>% 
  ggplot(aes(workclass, pct_over_50k)) + geom_bar(aes(alpha = log(n)),stat = 'identity') + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
