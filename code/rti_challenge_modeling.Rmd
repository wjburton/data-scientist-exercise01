...
plot_continuous <- function(variable, n = 30, response, out = F,
  ...
  if(log_odds == T){
    ...
    print(ggplot()+ geom_point(aes(x = association_tbl$point, y = association_tbl$prob_warm)) +
            ...
            geom_jitter(aes(x = df$variable, y = df$plot_point),
                        color = 'blue', alpha = .05, height = jitter_height, width = 0) +
            xlab(variable_name) + ylab('probability ofof making over 50k'))
  }
...
<br>
**Age** <br>
Here a third order polynomial of age is used to model whether a person is making over 50k or not
...
**hours_week**
```{r}
plot_continuous(train$hours_week, n = 200, response = train$over_50k, max_poly_degree = 4, log_odds = T, variable_name = 'hours_week')
